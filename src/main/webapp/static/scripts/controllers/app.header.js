...
//                         $scope.searchDealers.unshift({"siteId": 0, "id": 0, "dealerName": "All Dealers",selected:true});
                    ...
                    $scope.loadNewUrl = function () {
                        ...
                        if ($scope.getCurrentPage() === "dashboard") {
                            $state.go("app.dashboard.v1." + $scope.getCurrentTab(), {searchId: ($stateParams.searchId ? $stateParams.searchId : $scope.name.id), startDate: $('#startDate').val(), endDate: $('#endDate').val()});
                        } else if ($scope.getCurrentPage() === "report") {
                            //alert("GO");
                            $state.go("app.report.reports", {searchId: ($stateParams.searchId ? $stateParams.searchId : $scope.name.id), startDate: $('#startDate').val(), endDate: $('#endDate').val()});
                        } else if ($scope.getCurrentPage() === "dealer") {
                            $state.go("app.admin.dealer", {searchId: ($stateParams.searchId ? $stateParams.searchId : $scope.name.id), startDate: $('#startDate').val(), endDate: $('#endDate').val()});
                        } else if ($scope.getCurrentPage() === "conversion") {
                            $state.go("app.conversion.conversion", {searchId: ($stateParams.searchId ? $stateParams.searchId : $scope.name.id), startDate: $('#startDate').val(), endDate: $('#endDate').val()});
                        } else {
